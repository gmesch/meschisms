// Functions that are used to manage Ajax application state with the URL
// fragment identifier method.

// Manager for application state stored in the fragment identifier part of the
// page URL. Obtains the initial state of the application based on the current
// page URL.
function State() {
  // Map that says which state elements dominate which other. The keys are the
  // dominant elements, the values are arrays of properties that are recessive
  // against that element. If an element changes, all state elements that are
  // dominated by it are deleted.
  this.dominatedElements_ = {};

  // This variable contains the current state of the application in its external
  // representation, i.e.  as URL parameters. It is used by the history
  // detection function to detect history navigation: If the fragment identifier
  // of the URL of the page is different that the value stored in this object,
  // then a history navigation event occurred and the application is
  // notified. It is global in order to share it with the function that updates
  // the page URL to reflect changed application state: this must not be
  // mistaken for history navigation.
...
// Binds the State manager to its application and initializes the application
// from the current encoded state.
//
// loader A function that takes a state Object as argument and resets the
//   applications into the state represented by that object.
...
// Records the value of a state element in the hard or soft state part of the
// application state.
//
// This uses the global variable currentHash. This function updates the page
// URL, but we don't want the function that detects history navigation to false
// consider this a history navigation event. Therefore, we update the URL that
// the detectHistoryNavigation() function expects to see as the current URL of
// the page. This is accomplished by sharing the value through this object.
//
// @param {String} name The name of the state element.
//
// @param {String} value The value of the state element.
//
// @param {Boolean} hard If true, the state change is hard, i.e. it creates a
// new browser history step.
